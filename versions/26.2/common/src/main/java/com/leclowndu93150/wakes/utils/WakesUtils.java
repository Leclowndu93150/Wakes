...
import com.leclowndu93150.wakes.mixin.AbstractBoatAccessor;
    ...
    public static void spawnPaddleSplashCloudParticle(Level world, AbstractBoat boat) {
        ...
        for (int i = 0; i < 2; i++) {
            if (boat.getPaddleState(i)) {
                float paddleSpeed = AbstractBoatAccessor.wakes$paddleSpeed();
                double phase = ((AbstractBoatAccessor) boat).wakes$paddlePositions()[i] % (2*Math.PI);
                if (paddleSpeed / 2 <= phase && phase <= AbstractBoat.PADDLE_SOUND_TIME + paddleSpeed) {
...
//    }
    ...
    private static float getFluidLevel(Level world, int minX, int maxX, int minY, int maxY, int minZ, int maxZ) {
        // Taken from AbstractBoatEntity$getWaterHeightBelow
