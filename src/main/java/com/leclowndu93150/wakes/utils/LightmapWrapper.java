...
import com.leclowndu93150.wakes.duck.LightmapAccess;
import com.leclowndu93150.wakes.render.LightmapInfo;
...
public class LightmapWrapper {
    ...
    private static Vector3f clamp3(Vector3f color, double v, double v1) {
        return color.set(Mth.clamp(color.x, v, v1), Mth.clamp(color.y, v, v1), Mth.clamp(color.z, v, v1));
    }

    public static int readPixel(int block, int sky) {
        LightmapInfo info = ((LightmapAccess) Minecraft.getInstance().gameRenderer.lightTexture()).wakes$getLightmapInfo();
